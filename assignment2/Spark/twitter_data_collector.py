...
#
# This script is used to extract and reformat the data from original twitter data.
# ---------------------------------------------------------------------------------------------------------------------------------------
...
def collect_data(tweet):
	if tweet.get('geo') == None or tweet.get('geo').get('coordinates') == None or tweet.get('id') == None or tweet.get('created_at') == None:
		...
		geo = [float(coord) for coord in tweet['geo']['coordinates'] ]
		...
		tweet_info = Row(city = city, country = country, created_at = tweet['created_at'], 
							geo = geo, id = tweet['id'], lang = tweet['lang'],
...
if __name__ == "__main__":
	...
	if files == None:
		...
		# instead of logging into hdfs, we chose log on local file to make it more efficient:
		...
		for file in unlogged_files:
			...
			try:
				...
				with open('/vdc/team40/nia_test/collected/fail_log.txt','a') as my_fail:
					my_fail.write(file + '\n')
					my_fail.close()
