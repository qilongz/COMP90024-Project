---
title: "Exploration of Twitter Activity over Melbourne 2017"
...
insta <- read_delim("E:\\uni\\Cluster and Cloud Computing\\assign2\\TwitterExplore\\TwitterExplore\\bin\\location-2017.csv", ",", col_types = "nnn", quote = '', progress = FALSE)
...
why are there so many posts in here

```{r echo=FALSE,fig.height = 12,warning = FALSE}


region <- c(left = 144.945, right = 144.96, top = -37.79, bottom = -37.81)
innermelb <- filter(oz, region['left'] <= Xloc & Xloc <= region['right'],
region['top'] >= Yloc & Yloc >= region['bottom'])
...
stat_density_2d(data = pts, aes(x = Xloc, y = Yloc, fill = ..level..), geom = "polygon", alpha = 0.5) +
scale_fill_distiller(palette = "Spectral") +
geom_point(data = pts, aes(x = Xloc, y = Yloc,  alpha = Freq), color = 'navy') +
scale_alpha(trans = 'log10') + xlab('Longitude')+ylab('Latitude') +
theme(legend.position = "none")
