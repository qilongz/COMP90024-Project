...
namespace AnotateWithAurin
{
    ...
    public class CensusMedians
    {
        ...
        public static CensusMedians Extract(string aurinData, string xlst)
        {
            // extract data from aurin xml
            var data = Render(aurinData, xlst);
            var ser = new DataContractSerializer(typeof(CensusMedians));

            using (var xreader = data.CreateReader())
            {
                return (CensusMedians) ser.ReadObject(xreader, false);
            }
        }


        public static XDocument Render(string srcFile, string xslFile)
        {
